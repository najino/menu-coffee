...
import { ApiBearerAuth, ApiBody, ApiConsumes, ApiCreatedResponse, ApiExtraModels, ApiNotFoundResponse, ApiOkResponse, ApiParam, ApiQuery, ApiResponseProperty, ApiTags, getSchemaPath } from '@nestjs/swagger';
...
export class ProductController {
  ...
  @IsAuth()
  createProduct(@Body() createProductDto: CreateProductDto, @UploadedFile("file", FilePipeBuilder()) img: Express.Multer.File) {
  ...
  @ApiQuery({ name: "limit", description: "limit Of Product You want to fetched By Default(10)", required: false })
  @ApiQuery({ name: "page", description: "page you want to see Default(1)", required: false })
  ...
  @ApiNotFoundResponse({ description: "Product not found", type: HttpExceptionDto })
  @ApiTags("Admin")
  @ApiBody({
    schema: {
      allOf: [
  ...
  @IsAuth()
  @UseInterceptors(FileInterceptor("file"))
  update(@Param() { id }: MongoIdDto, @Body() updateProductDto: UpdateProductDto, @UploadedFile("file", FilePipeBuilder(false)) img?: Express.Multer.File) {
  ...
  @ApiOkResponse({ description: "Product remove successfully", type: ProductDto })
  @ApiNotFoundResponse({ description: "Product Not found", type: HttpExceptionDto })
  @IsAuth()
